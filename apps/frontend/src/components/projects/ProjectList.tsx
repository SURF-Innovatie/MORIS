...
const getProjectStatus = (project: ProjectResponse) => {
    if (!project.startDate || !project.endDate) return { label: "Unknown", variant: "secondary" as const };
...
export const ProjectList = ({ showCreateButton = true }: ProjectListProps) => {
    ...
    const { data: projects, isLoading: isLoadingProjects, error: projectsError } = useGetProjects();
            ...
            {projects && projects.length > 0 && viewMode === "cards" && (
                <div className="grid gap-6 sm:grid-cols-2">
                    {projects.map((project) => {
                        ...
                        return (
                            <Card key={project.id} className="group flex flex-col transition-all hover:shadow-md hover:border-primary/20" onClick={() => navigate(`/projects/${project.id}/edit`)}>
                                        ...
                                        {project.people && project.people.length > 0 ? (
                                            <>
                                                {project.people.slice(0, 4).map((person, i) => (
                                                    <div
                                                        key={i}
                                                        className="h-6 w-6 rounded-full ring-2 ring-background bg-muted flex items-center justify-center text-[10px] font-medium text-muted-foreground uppercase text-center"
                                                        title={person.email}
                                                    >
                            ...
                            <TableBody>
                                {projects.map((project) => (
                                    <TableRow key={project.id}>
                                        ...
                                        <TableCell>
                                            <Badge variant={getProjectStatus(project).variant} className="text-[10px] h-5">
