...
import { usePutPeopleId, useGetUsersUserIdPages } from "@api/moris";
...
import { Pencil, LayoutTemplate, Eye, Plus } from "lucide-react";
...
export function ProfileInfo({ user, refetchProfile }: ProfileInfoProps) {
  ...
  const { data: pages } = useGetUsersUserIdPages(user.id || "", {
    query: {
      enabled: !!user.id,
    },
  });
          ...
          <div className="flex items-center gap-2">
            {existingPage ? (
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  navigate(`/pages/new?type=profile&userId=${user.id}`)
                }
              >
