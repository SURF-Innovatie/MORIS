...
export function AffiliatedOrganisationsTab({
  ...
  async function addFromVat() {
    ...
    try {
      const newOrg = await createOrganisation({
        data: {
          name: (vatResult as any).name || "Unknown",
          vatNumber: (vatResult as any).vat_number,
          city: (vatResult as any).city || "",
          country: (vatResult as any).country_code || "",
        },
      });
        ...
        <DialogContent className="sm:max-w-[600px]">
          ...
          <Tabs value={activeSearchTab} onValueChange={setActiveSearchTab}>
            ...
            <TabsContent value="vat" className="space-y-4 mt-4">
              ...
              {(vatResult as any) && (vatResult as any).valid && (
                <div className="space-y-3">
                  ...
                  <div className="p-4 border rounded-lg space-y-2">
                    <h4 className="font-medium">{(vatResult as any).name}</h4>
                    <p className="text-sm text-muted-foreground whitespace-pre-line">
                      {(vatResult as any).address?.trim()}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      VAT: {(vatResult as any).vat_number}
                    </p>
                  </div>
                  <Button
                    onClick={addFromVat}
