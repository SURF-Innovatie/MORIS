...
function EditMemberCustomFieldsButton({
    ...
    const { data: fields, isLoading: isLoadingFields } = useGetOrganisationNodesIdCustomFields(nodeId, { category: "PERSON" });
    const { mutateAsync: updateFields, isPending } = usePutOrganisationNodesIdMembersPersonIdCustomFields();
    ...
    const onSubmit = async (data: any) => {
        try {
            ...
            toast({ title: "Failed to update custom fields", variant: "destructive" });
        }
    };

    if (!membership.person?.id) return null;

    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button variant="ghost" size="icon" title="Edit Custom Fields">
                    <Pencil className="h-4 w-4 text-gray-500" />
                </Button>
            </DialogTrigger>
            <DialogContent className="max-w-lg">
                <DialogHeader>
                    <DialogTitle>Edit Custom Fields for {membership.person.name}</DialogTitle>
                </DialogHeader>
                                ...
                                {field.type === "BOOLEAN" ? (
                                     <div className="flex items-center space-x-2">
                                        <Checkbox 
                                            ...
                                            {...register(field.id)}
                                        />
                                        <label htmlFor={field.id} className="text-sm text-muted-foreground">{field.description}</label>
                                     </div>
                                ) : (
                                    <Input 
