...
func main() {
	...
	if err != nil {
		...
		log.Fatal().Err(err).Msgf("Could not read swag-generated spec from %s. Custom endpoints might be missing from final OpenAPI.", swagSpecPath)
	}
	...
	if err := json.Unmarshal(swagBytes, &meta); err != nil {
		log.Fatal().Err(err).Msg("failed to inspect swag-generated spec version")
	}
	...
	case strings.HasPrefix(meta.Swagger, "2."):
		...
		if err := json.Unmarshal(swagBytes, &swaggerDoc); err != nil {
			log.Fatal().Err(err).Msg("failed to unmarshal swagger v2 spec")
		}
		openapi3Doc, err := openapi2conv.ToV3(&swaggerDoc)
		if err != nil {
			log.Fatal().Err(err).Msg("failed to convert swagger v2 spec to openapi v3")
		}
		convertedBytes, err := openapi3Doc.MarshalJSON()
		if err != nil {
			log.Fatal().Err(err).Msg("failed to marshal converted openapi spec")
		}
		if err := json.Unmarshal(convertedBytes, swagSpec); err != nil {
			log.Fatal().Err(err).Msg("failed to load converted openapi spec")
		}
		log.Info().Msgf("INFO: Converted swagger v2 spec from %s to OpenAPI v3", swagSpecPath)
	case strings.HasPrefix(meta.OpenAPI, "3."):
		if err := json.Unmarshal(swagBytes, swagSpec); err != nil {
			log.Fatal().Err(err).Msg("failed to unmarshal openapi v3 spec")
		}
		log.Info().Msgf("INFO: Loaded OpenAPI v%s spec from %s", meta.OpenAPI, swagSpecPath)
	default:
		if err := json.Unmarshal(swagBytes, swagSpec); err != nil {
			log.Fatal().Err(err).Msg("failed to unmarshal swagger spec with unknown version")
		}
		log.Info().Msgf("WARN: Swagger spec version not detected; attempting to use raw document from %s", swagSpecPath)
	}
	...
	if err := os.MkdirAll(filepath.Dir(specOutputPath), 0o755); err != nil {
		log.Fatal().Err(err).Msg("failed to ensure OpenAPI output directory")
	}
	specFile, err := os.Create(specOutputPath)
	if err != nil {
		log.Fatal().Err(err).Msg("failed to create OpenAPI output file")
	}
	defer func() {
		if cerr := specFile.Close(); cerr != nil {
			log.Warn().Err(cerr).Msg("WARN: failed to close OpenAPI output file")
		}
	}()
	...
	ex, err := entoas.NewExtension(
		...
		entoas.Mutations(func(graph *gen.Graph, entoasSpec *ogen.Spec) error {
			...
			if swagSpec.Paths != nil {
				...
				log.Info().Msgf("INFO: Merged %d custom paths from swag-generated spec.", len(swagSpec.Paths))
			}
			...
			if swagSpec.Components != nil {
				...
				if len(swagSpec.Components.Schemas) > 0 {
					log.Info().Msgf("INFO: Merged %d custom schemas/components from swag-generated spec.", len(swagSpec.Components.Schemas))
				}
			}
	...
	if err != nil {
		log.Fatal().Err(err).Msg("creating entoas extension")
	}
	...
	if err != nil {
		log.Fatal().Err(err).Msg("running ent code generation")
	}

	log.Info().Msg("Ent code and OpenAPI spec generated successfully, including custom endpoints from swag.")
}
