...
type UserCache interface {
	...
	SetUser(ctx context.Context, user *identity.User) error
	DeleteUser(ctx context.Context, id uuid.UUID) error
}

type RedisUserCache struct {
	*RedisCache[identity.User]
}

func NewRedisUserCache(rdb *redis.Client, ttl time.Duration) *RedisUserCache {
	return &RedisUserCache{
		RedisCache: NewRedisCache[identity.User](rdb, ttl),
	}
}
...
func (c *RedisUserCache) GetUser(ctx context.Context, id uuid.UUID) (*identity.User, error) {
	return c.Get(ctx, c.key(id))
}

func (c *RedisUserCache) SetUser(ctx context.Context, user *identity.User) error {
	if user == nil {
