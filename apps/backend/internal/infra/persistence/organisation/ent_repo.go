...
func (r *EntRepo) CreateNode(ctx context.Context, name string, parentID *uuid.UUID) (*entities.OrganisationNode, error) {
	create := r.node().Create().SetName(name)
...
func (r *EntRepo) UpdateNode(ctx context.Context, id uuid.UUID, name string, parentID *uuid.UUID) (*entities.OrganisationNode, error) {
	upd := r.node().UpdateOneID(id).SetName(name)
