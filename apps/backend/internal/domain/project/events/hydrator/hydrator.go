...
type OrgNodeLoader interface {
	OrganisationNodesByIDs(ctx context.Context, ids []uuid.UUID) (map[uuid.UUID]organisation.OrganisationNode, error)
}
...
func (h *Hydrator) loadPersons(ctx context.Context, ids []uuid.UUID) map[uuid.UUID]identity.Person {
	if len(ids) == 0 {
		return nil
	}
...
func (h *Hydrator) loadRoles(ctx context.Context, ids []uuid.UUID) map[uuid.UUID]role.ProjectRole {
	if len(ids) == 0 {
		return nil
	}
...
func (h *Hydrator) loadProducts(ctx context.Context, ids []uuid.UUID) map[uuid.UUID]product.Product {
	if len(ids) == 0 {
		return nil
	}
...
func (h *Hydrator) loadOrgNodes(ctx context.Context, ids []uuid.UUID) map[uuid.UUID]organisation.OrganisationNode {
	if len(ids) == 0 {
		return nil
	}
...
func (h *Hydrator) loadCreators(ctx context.Context, ids []uuid.UUID) map[uuid.UUID]identity.Person {
	if len(ids) == 0 {
		return nil
	}
