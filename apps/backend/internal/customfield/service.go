...
type Service interface {
	Create(ctx context.Context, orgID uuid.UUID, name, fieldType string, description, validationRegex, exampleValue *string, required bool) (*ent.CustomFieldDefinition, error)
	Delete(ctx context.Context, id uuid.UUID, orgID uuid.UUID) error
	ListAvailableForNode(ctx context.Context, orgID uuid.UUID) ([]*ent.CustomFieldDefinition, error)
...
func (s *service) Create(ctx context.Context, orgID uuid.UUID, name, fieldType string, description, validationRegex, exampleValue *string, required bool) (*ent.CustomFieldDefinition, error) {
	creator := s.cli.CustomFieldDefinition.Create().
		SetOrganisationNodeID(orgID).
		SetName(name).
		SetType(customfielddefinition.Type(fieldType)).
...
func (s *service) ListAvailableForNode(ctx context.Context, orgID uuid.UUID) ([]*ent.CustomFieldDefinition, error) {
	...
	// Query definitions
	return s.cli.CustomFieldDefinition.Query().
		Where(customfielddefinition.OrganisationNodeIDIn(validOrgIDs...)).
		Order(ent.Asc(customfielddefinition.FieldName)).
		All(ctx)
}
